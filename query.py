...
def search():
  ...
  for vector_id in all_ids:
    ...
    llm=llm,  
    chain_type="stuff",  
    retriever=pinecone.as_retriever(search_kwargs={"filter": {"id":vector_id}})
    )  
    
    #Candidate Name
    print("Name of Candidate")
    query=f"Give ME Candidate Name(Only name nothing else)"
  ...
  #   print(result1)
    ...
    #for skills comparesion
    query = f"compare resume skills with [{user_skills}] and give me the number of matching skills(like this* **HTML:** Match,*CSS:** Match with ID"
    ...
    #for work experinece
    query ="give me experience of candate like(compnyname:xyx,position:xyz,duration:time) with"
    response4 = qa.invoke(query)
    result4  = response4['result'].strip()
    print("\n")
